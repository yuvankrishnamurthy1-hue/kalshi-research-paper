...
OUT_DIR = os.path.join(HERE, "kalshi_api_data_h2farexpiry")
RAW_DIR = os.path.join(OUT_DIR, "raw")
SUMMER_RAW_DIR = os.path.join(HERE, "kalshi_api_data_summer", "raw")
EXTENDED_RAW_DIR = os.path.join(HERE, "kalshi_api_data_extended", "raw")
SLEEP = 0.5
...
EVENTS = [
    # H2 sports->macro re-pull (D12 fix): SAME 15 unscheduled_sports events,
    # but FAR-EXPIRY macro contracts only (close_time > T0+60d). The v2 A2 batch
    # used nearest-expiry buckets whose moves were mechanical pull-to-par.
    # T0s from hidden_files/event_registry.csv.
    ("myles_garrett_to_rams_fx", "2026-06-01T17:19:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("aj_brown_to_patriots_fx", "2026-06-01T20:20:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("giannis_to_heat_fx", "2026-06-23T03:50:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("jaylen_brown_to_76ers_fx", "2026-07-01T22:20:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("mike_conley_to_celtics_fx", "2026-07-01T14:58:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("mitchell_robinson_to_celtics_fx", "2026-07-01T15:39:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("neemias_queta_extension_fx", "2026-07-03T13:53:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("donovan_mitchell_extension_fx", "2026-07-07T10:01:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("dlo_six_team_trade_fx", "2026-07-08T00:53:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("jordan_walsh_extension_fx", "2026-07-23T12:27:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("lebron_to_76ers_fx", "2026-07-24T20:34:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("brian_oneill_extension_fx", "2026-07-28T14:24:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("von_miller_to_cowboys_fx", "2026-08-17T01:10:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("aaron_donald_comeback_fx", "2026-08-30T17:00:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
    ("kawhi_to_raptors_fx", "2026-09-14T18:56:00Z", "unscheduled_sports",
     "crossdomain_macro_farexpiry", ["KXFED", "KXCPI"],
     {"select": "far_expiry", "n_per_series": 2, "min_expiry_days": 60}),
]
...
def select_far_expiry(relevant, t0, n_per_series, min_days=60):
    """relevant: [(ticker, market, dsrc)] for ONE series. Keep the n_per_series
    contracts whose close_time is strictly beyond t0 + min_days (far-expiry:
    no pull-to-par convergence inside the [T-45, T+10] window), nearest such
    expiry first. Shortfalls are returned for snag logging."""
    cutoff = (t0 + timedelta(days=min_days)).isoformat()
    far = sorted([x for x in relevant if x[1].get("close_time", "") > cutoff],
                 key=lambda x: x[1].get("close_time", ""))
    return far[:n_per_series], far[n_per_series:], max(0, n_per_series - len(far))
...
def load_cached_candles(ticker, start_ts, end_ts):
    ...
    for raw_dir in (RAW_DIR, EXTENDED_RAW_DIR, SUMMER_RAW_DIR):
...
def main():
    ...
    log = {"run": "h2farexpiry", "events": [], "tickers": {}}
    meta_rows, data_rows = [], []
    candle_cache = {}
    ...
    for event_id, t0_iso, cell, category, series_list, opts in EVENTS:
        ...
        for series in series_list:
            ...
            if sel == "nearest_expiry":
                ...
                relevant, _extra, short = select_far_expiry(
                    relevant, t0, opts.get("n_per_series", 2),
                    opts.get("min_expiry_days", 60))
                if short:
                    ev_log.setdefault("far_expiry_short", []).append(
                        {"series": series, "requested": opts.get("n_per_series", 2),
                         "got": len(relevant),
                         "cutoff": (t0 + timedelta(days=opts.get("min_expiry_days", 60))).date().isoformat()})
                    print(f"  !! {series}: only {len(relevant)} far-expiry contracts "
                          f"(wanted {opts.get('n_per_series', 2)})", flush=True)
