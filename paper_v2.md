...
5. **Macro news barely leaks into sports markets** (+0.16¢ — real but ~3% of the own-market effect). **Sports news doesn't leak into macro markets either, as far as we can now test:** daily data couldn't do it (two failed attempts), but minute-by-minute prices around each announcement can — in the 6 cases where the macro contract was actually trading, the average move at announcement time was −0.5¢ (statistically zero, p ≈ 0.20), and the one −2¢ wiggle was the FOMC meeting happening the same day, not the sports news.
...
**But D16 plus the extended sample makes H1b testable for the first time:** adding the draft, lottery, and All-Star Game takes scheduled sports to 5 events. Result: 15 surprise (−0.15¢) vs 5 scheduled (+0.38¢), p = 0.40 — **not significant**. The first real test of "do surprise roster moves move markets more than scheduled sports events" comes back null. (This uses post-plan data additions, so it's labeled accordingly. One of the five — the draft lottery, +2.62¢ — is flagged as confounded by concurrent playoff games, and two of the three added events had their T0s corrected by a day after verification; neither change moves the result.)
...
### 6. Cross-domain check (H2): leakage is economically nil both ways
...
- **Sports news → macro markets: tested intraday, no effect (D23).** Daily data failed twice (D12: nearest-expiry contracts gave mechanical garbage; D15: far-expiry contracts barely trade and a June Fed repricing manufactured fake moves). Kalshi's API serves minute candles, so we re-ran it properly: for each of the 15 sports events, minute-by-minute prices of the most-traded macro contract in a ±3h window around the announcement, measuring the move from the hour before to the hour after. 24 of 30 event–contract pairs are still untestable — no trades, stub quotes with 13–58¢ spreads, no real price that far from macro releases. But 6 pairs have genuine price discovery, and the announcement-time jumps are: 0.0¢, −1.0¢, 0.0¢, 0.0¢, −2.0¢, 0.0¢ — mean −0.5¢, t = −1.46 vs zero, p ≈ 0.20. The −2.0¢ is same-day FOMC-meeting drift (the contract sat flat through the announcement, then slid for two hours — wrong shape for a news jump, on an offensive-lineman extension). A seventh qualitative check: 47,000 Fed contracts traded on the D'Angelo Russell trade night with the price pinned flat across the announcement. Where macro prices are real, sports news doesn't move them — consistent with H2's prediction of no sports→macro leakage. Caveat: n = 6 is small and the testable subset is picked by the macro calendar, so this is a completed test with limited power, not a high-precision zero.
...
1. **The September cluster.** Three macro observations share a date, the same contracts, and the FOMC decision; they're not independent. The macro leg is effectively ~6 event-days, not 8 events. The cluster-collapsed sensitivity (D22) makes both headline contrasts *weaker*, not stronger: H1c goes from p = 0.077 to p = 0.34 (macro +3.21¢ vs sports +1.79¢), H1a from p = 0.17 to p = 0.57. The macro edge was partly triple-counted September 16.
...
6. **Resolution.** Daily candles for everything except the H2 sports→macro leg, which uses minute candles in ±3h windows (D23). Several macro events have date-only timestamps (±1 day on day-0). No intraday *speed* claims.
