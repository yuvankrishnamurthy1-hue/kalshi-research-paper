...
SUFFIX         = os.environ.get("KALSHI_SUFFIX", "")  # D16: e.g. "_d16"
...
MIN_BASELINE_N = 3            # D16: minimum baseline obs in T-30..T-5 (was 5); <5 flagged thin
...
def analyze_pair(ev_row, md, window_days):
    ...
    # D16 (amendment, see prereg_deviations.md): primary window stays T-30..T-5
    # but needs only 3 obs (<5 flagged thin_baseline). If <3 obs, extended
    # fallback uses the full [T-30,T-1] pre-window when it has >=5 obs
    # (flagged extended_baseline; nearer-event days may carry rumor run-up).
    # Events with <5 total pre-T0 obs stay excluded. D16 supersedes FIX 7's
    # T-45..T-5 retry.
    base = baseline_slice(px, t0, *BASELINE_DAYS)["mid"].dropna()
    baseline_rule = "strict"
    fallback = False
    if len(base) < MIN_BASELINE_N:
        ext = px[(px["ts"] >= t0 - timedelta(days=30)) &
                 (px["ts"] < t0 - timedelta(days=1))]["mid"].dropna()
        if len(ext) >= 5:
            base = ext
            baseline_rule = "extended_d16"
            fallback = True
        else:
            return exclude("insufficient_baseline_history_d16")
    elif len(base) < 5:
        baseline_rule = "thin_d16"
    base_mean = base.mean()
    ...
    return {
        ...
        "thin_baseline": baseline_rule != "strict",
...
def aggregate_by_event(df):
    ...
    for (eid, cell, w), g in df.groupby(["event_id", "cell", "window_days"]):
        ...
        mode = g["tier"].mode()
        rmode = g["baseline_rule"].mode() if "baseline_rule" in g else pd.Series([])
        rows.append({
            "event_id": eid,
            "cell": cell,
            "window_days": w,
            "baseline_rule": rmode.iloc[0] if len(rmode) else "n/a",
...
def main():
    ...
    df.to_csv(f"{OUT_DIR}/pair_results{SUFFIX}.csv", index=False)
    # FIX 3: event-level results are the primary analysis unit.
    dev_all = aggregate_by_event(df)
    dev_all.to_csv(f"{OUT_DIR}/event_results{SUFFIX}.csv", index=False)
    ...
    if EXCLUSIONS:
        pd.DataFrame(EXCLUSIONS).to_csv(f"{OUT_DIR}/exclusions{SUFFIX}.csv", index=False)
    ...
    for label, d in [("full", df), ("liquid_only", df[df["liquid"]])]:
        ...
        pooled.to_csv(f"{OUT_DIR}/pooled_{label}_{HEADLINE_WINDOW}d{SUFFIX}.csv", index=False)
        kw.to_csv(f"{OUT_DIR}/tier_kruskal_{label}_{HEADLINE_WINDOW}d{SUFFIX}.csv", index=False)
        con = factorial_contrasts(d, HEADLINE_WINDOW)
        con.to_csv(f"{OUT_DIR}/contrasts_{label}_{HEADLINE_WINDOW}d{SUFFIX}.csv", index=False)
        h2 = h2_cross_domain(d, HEADLINE_WINDOW)
        h2.to_csv(f"{OUT_DIR}/h2_cross_domain_{label}_{HEADLINE_WINDOW}d{SUFFIX}.csv", index=False)
    ...
    psumm.to_csv(f"{OUT_DIR}/placebo_summary{SUFFIX}.csv", index=False)
